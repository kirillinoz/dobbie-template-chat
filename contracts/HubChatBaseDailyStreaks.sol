...
contract HubChatBaseDailyStreaks {
    ...
    address public constant HUB_TOKEN_ADDRESS =
        0x58EFDe38eF2B12392BFB3dc4E503493C46636B3E;

    string public welcomeMessage =
        "Daily USDC reward claimed! Thank you for using HUB Chat!";
    string public cooldownMessage =
        "Please wait 24 hours between claims. Stay active in HUB Chat!";
    string public blacklistMessage =
        "This address cannot claim rewards. Contact HUB Chat support.";
    string public insufficientHubMessage =
        "You need at least 10 HUB tokens to claim USDC reward, but your streak is saved!";
    string public streakSavedMessage =
        "Streak saved! Get 10 HUB tokens to claim USDC rewards.";
    ...
    modifier notBlacklisted() {
        require(
            !blacklist[msg.sender] && !userStats[msg.sender].isBlocked,
            blacklistMessage
        );
        _;
    }
    ...
    function claim() external notBlacklisted {
        ...
        require(
            block.timestamp >= stats.lastClaimTime + CLAIM_COOLDOWN,
            cooldownMessage
        );
        ...
        if (canClaimUSDC) {
            uint256 contractUSDCBalance = IERC20(USDC_ADDRESS).balanceOf(
                address(this)
            );
            require(
                contractUSDCBalance >= DAILY_USDC_REWARD,
                "Insufficient USDC funds in contract"
            );
            ...
            string memory streakMessage = string(
                abi.encodePacked(
                    welcomeMessage,
                    " Current streak: ",
    ...
    function canClaim(address user) public view returns (bool, string memory) {
        if (blacklist[user] || userStats[user].isBlocked)
            return (false, blacklistMessage);
        ...
        if (stats.lastClaimTime == 0) {
            if (hasEnoughHUB) {
                ...
                return (
                    true,
                    "Welcome! Claim to start your streak. Get 10 HUB tokens to earn USDC!"
                );
            }
        }
        ...
        if (stats.lastClaimTime == 0) {
            if (hasEnoughHUB) {
                return (
                    ...
                    0,
                    0,
                    "Welcome! Claim your first daily USDC!",
                    hasEnoughHUB
                );
            } else {
                return (
        ...
        )
    {
        ...
        hasEnoughHUB =
            IERC20(HUB_TOKEN_ADDRESS).balanceOf(user) >= MIN_HUB_REQUIRED;
    }
