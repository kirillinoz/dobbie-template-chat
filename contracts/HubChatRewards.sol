...
contract HubChatRewards {
    address public constant HUB_TOKEN =
    ...
    string public welcomeMessage =
        "Welcome to HUB Chat! Earn HUB Ecosystem tokens for messages!";
    string public blacklistMessage =
        "This address cannot send messages. Contact HUB Chat support.";
    ...
    function sendMessage(
    ...
    ) external whenNotPaused notBlocked {
        require(
        ...
        require(
            getContractHUBBalance() >= REWARD_PER_MESSAGE,
            "Insufficient HUB tokens in contract"
        );

        bool success = IERC20(HUB_TOKEN).transfer(
            msg.sender,
            REWARD_PER_MESSAGE
        );
    ...
    function depositHUBTokens(uint256 amount) external onlyOwner {
        ...
        bool success = IERC20(HUB_TOKEN).transferFrom(
            msg.sender,
            address(this),
            amount
        );
